    ...
    static boolean loadAcqConfig(String dbHost,String dbName,String dbUser,String dbPassword,String redisHost){
        ...
        try {
            ...
            while(rs.next()){
                ...
                if(null == cleanRule) cleanRule ="null";
                switch (cleanRule){
                    case "null": {
                        cleanRule = "";
                        break;
                ...
                if(null == calculateRule) calculateRule ="null";
                ...
                if(null == saveRule) saveRule ="null";
                switch (saveRule){
                    case "null": {
                        saveRule = "0";
                        break;
                ...
                if(null == timeoutRule) timeoutRule ="null";
                switch (timeoutRule){
                    case "null": {
                        timeoutRule = "-1";
                        break;
...
//        ///////////////////////////////////////////////////////////////////////////////////////////
        builder.setBolt("data-process-bolt",new DataProcessBolt() ,GroupingMethod.dataCalculateBoltCount)
                .customGrouping("rtdata-message-reader-spout","calculate-data",new TagIdGrouping())
                .customGrouping("hisdata-message-reader-spout","calculate-data",new TagIdGrouping())
                .customGrouping("acq-config-bolt","calculate-data",new TagIdGrouping());
...
//        nextTuple可以以block方式获取数据，比如从disruptor中或BlockingQueue中获取数据，当没有数据时，直接block住，节省了大量CPU。
        ...
        if(debugFlag)
        {
            ...
            cluster.submitTopology("ssssss", conf, builder.createTopology());
            Thread.sleep(60000000);
            cluster.killTopology("ssssss");//VER
            cluster.shutdown();
        }
       else{
            try {
